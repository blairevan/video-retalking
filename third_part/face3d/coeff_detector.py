...
class CoeffDetector(nn.Module):
    ...
    def forward(self, img, lm):
        ...
        self.model.test()
        pred_coeff = {key: self.model.pred_coeffs_dict[key].cpu(
        ).numpy() for key in self.model.pred_coeffs_dict}
    ...
    def image_transform(self, images, lm):
        ...
        trans_params = np.array([float(item)
                                for item in np.hsplit(trans_params, 5)])
...
def get_data_path(root, keypoint_root):
    ...
    IMAGE_EXTENSIONS = IMAGE_EXTENSIONS_LOWERCASE.union(
        {f.upper() for f in IMAGE_EXTENSIONS_LOWERCASE})
...
if __name__ == "__main__":
    ...
    image_names, keypoint_names = get_data_path(
        opt.input_dir, opt.keypoint_dir)
